...
Dataset::Dataset(std::string dataset_path):
    ...
    if (myRgbFile.is_open() && myDepthFile.is_open() && myGroundtruthFile.is_open())
    {
        ...
        //std::cout << "RGBD pairs: " << timestamp_rgbd_.size() << std::endl;
        ...
        for (int i = 0; i < timestamp_rgbd_.size(); ++i){
            for (int j = 0; j < timestamp_groundtruth_.size(); ++j){
                ...
                if(abs(a - b) < max_difference){
                    std::tuple<double,int,int> t(abs(a - b),i,j);
                    ternas.push_back(t);
                }
            }
        } // Fin de bucle FOR
        ...
        for (int i = 0; i < ternas.size(); ++i){
            ...
            if( it_a == tempStamps.end() &&  it_b == tempStamps_gt.end()){
                //std::cout << "rgbd: " << rgb_filenames_.at(a) << " gt: " << timestamp_groundtruth_.at(b) << std::endl;
        ...
        for (int frame = 0; frame < matches.size(); ++frame){
            int ts_rgbd_index = std::get<0>(matches[frame]);
            int ts_gt_index = std::get<1>(matches[frame]);
            ...
            addFrame(num_frames_,
                     img_rgb,
                     img_depth,
                     timestamp_rgbd_[ts_rgbd_index],
                     timestamp_groundtruth_[ts_gt_index],
                     poses_gt_[ts_gt_index]);
    ...
    dataset_path_(dataset_path)
{
    ...
    trainFile.open(dataset_path_ + "/TrainSplit_copy.txt");
    testFile.open(dataset_path_ + "/TestSplit_copy.txt");
    ...
    if(trainFile.is_open() && testFile.is_open())
    {
        ...
        // Leyendo el Training Data
        for (int i = 0; i < train_sequences.size(); ++i)
        {
            int seq = train_sequences[i];
            ...
            for (int frame = 0; frame < 1000; ++frame)
            {
                ...
                s_rgb = dataset_path_ + "/seq-0"+ std::to_string(seq) +"/frame-" + s + ".color.png";
                s_depth = dataset_path_ + "/seq-0"+ std::to_string(seq) +"/frame-" + s + ".depth.png";
                s_pose = dataset_path_ + "/seq-0"+ std::to_string(seq) +"/frame-" + s + ".pose.txt";
            ...
            std::cout << "Secuencia " << seq << " Cargada\n";

        } // Fin de FOR // Lectura de todas subsecuencias

        // Leyendo el Test Data
        for (int i = 0; i < test_sequences.size(); ++i)
        {
            int seq = test_sequences[i];
            // Leemos los paquetes de los frames
            for (int frame = 0; frame < 1000; ++frame)
            {
                // Construyendo los strings para la lectura
...
void Dataset::addFrame(int num_frame, cv::Mat rgb_frame, cv::Mat depth_frame, std::string timestamp_rgbd, std::string timestamp_gt, Pose pose){
    rgb_images_.insert(std::pair<uint32_t, cv::Mat>(num_frame, rgb_frame));
    depth_images_.insert(std::pair<uint32_t, cv::Mat>(num_frame, depth_frame));
    poses_.insert(std::pair<uint32_t, Pose>(num_frame, pose));
    timestamp_.push_back(timestamp_gt);
    timestamp_rgbd_sync_.push_back(timestamp_rgbd);
    num_frames_++;
}

bool Dataset::check_timestamp_rgbd_match(const std::string &s, int &idx)
{
    std::vector<std::string>::iterator it;

    it = std::find(timestamp_rgbd_sync_.begin(),timestamp_rgbd_sync_.end(),s);

    if(it != timestamp_rgbd_sync_.end())
    {
        idx  = std::distance(timestamp_rgbd_sync_.begin(), it);
        return true;
    }
    return false;
